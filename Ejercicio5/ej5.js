...
let nombre = window.prompt("Veamos tu nombre de super héroe/heroina, primero dime tu nombre").toLowerCase();
let apellido = window.prompt("Ahora dime tu primer apellido:").toLowerCase();
let sexo = parseInt(
  window.prompt(`Introduce tu sexo Femenino(1), Masculino(2)`));
let fecha = window.prompt("Introduce tu fecha de nacimiento (DD-MM-AAAA):");

let letraN = nombre.substring(0, 1);
let letraA = apellido.substring(apellido.length - 1);
let numeroFecha = parseInt(fecha.substring(fecha.length - 1));
